...
public class MenuUtils {
	...
	public static MenuInventory createMenu(String title, int rows, String menuTag, HangoutPlayer p){
		MenuInventory menu = new MenuInventory(Bukkit.createInventory(null, rows * 9, title), menuTag);
	...
	public static MenuInventory createMainMenu(HangoutPlayer p){
		MenuInventory inventory = createMenu("Main menu", 3, "main_menu", p);
	...
	public static MenuInventory createSettingsMenu(HangoutPlayer p){
		MenuInventory inventory = createMenu("Settings", 3, "settings_menu", p);
		...
		for(ChatChannel c : ChatManager.getChannels()){
			...
			createMenuItem(inventory, c.getMaterial(), c.getDisplayName(), c.getDescription(), 0 + channelCount, "channel_description_" + c.getTag());
			if(isSubscribed){
